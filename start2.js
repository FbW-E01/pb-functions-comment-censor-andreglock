function censor(comments) {
    for (let i = 0; i < comments.length; i++) {
        ...
        let filter = comments[i].toLowerCase().replace('heck', 'H**k');

        if (filter != comments[i].toLowerCase()) {
            comments[i] = filter;
        }

        filter = comments[i].toLowerCase().replace('feck', 'F*ck');

        if (filter != comments[i]) {
            comments[i] = filter;
        }

        filter = comments[i].toLowerCase().replace('darn', `Heckn'`);

        if (filter != comments[i]) {
            comments[i] = filter;
        }

        filter = comments[i].toLowerCase().replace('assassin', 'WooohooWoohooin');

        if (filter != comments[i]) {
            comments[i] = filter;
        }

        filter = comments[i].toLowerCase().replace('dude', 'Friendo');
